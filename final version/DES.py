...
def encode_msg_SDES(plaintxt , key):
...
def decode_msg_SDES(ciphertxt , key):
...
#takes in 8 bits
def Encrypt_triple_SDES(txt , key):
    assert(len(txt) == 8)
    txt = Encrypt_Simplified_DES(txt , key)
    txt = Encrypt_Simplified_DES(txt , key)
    txt = Encrypt_Simplified_DES(txt , key)
    return txt

def Decrypt_triple_SDES(txt , key):
    assert(len(txt) == 8)
    txt = Decrypt_Simplified_DES(txt , key)
    txt = Decrypt_Simplified_DES(txt , key)
    txt = Decrypt_Simplified_DES(txt , key)
    return txt

def encode_triple_SDES(txt , key):
    byte_encoding = []
    for i in range(len(txt)):
        bin_letter = letter_to_bin(txt[i])
        byte_encoding.append(Encrypt_triple_SDES(bin_letter , key))
    return " ".join(byte_encoding)    

def decode_triple_SDES(ciphertxt , key):
    encoded_letters = ciphertxt.split()
    decoded_letters = []
    for encoded_letter in encoded_letters:
        bin_decoded_letter = Decrypt_triple_SDES(encoded_letter , key)
        decoded_letter = bin_to_letter(bin_decoded_letter)
        decoded_letters.append(decoded_letter)
    return "".join(decoded_letters)
...
def test():
    key = '1100011110'
    c_txt = encode_msg_SDES("hello world" , key)
    decode_msg_SDES(c_txt , key)
...
if __name__ == "__main__":
    ...
    ciphertxt = encode_msg_SDES(plaintxt , key)
    print("plaintext is" , plaintxt)
    print("Encoded message is" , ciphertxt)
    decrypted_txt = decode_msg_SDES(ciphertxt , key)
    print("decrypted message is" , decrypted_txt)
    
    #some fun stuff
    ciphertxt2 = encode_msg_SDES("Hello agent bob, you have a top secret mission." , "1001100001")
    decrypted_txt = decode_msg_SDES(ciphertxt2 , "1001100001")
